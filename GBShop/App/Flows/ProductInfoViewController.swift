...
class ProductInfoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupViews()
        getReviews(productId: self.product.id) { arrr in
            self.reviewList = arrr
    ...
    private func getReviews(productId: Int, completion: ([ReviewUser]) -> () ) { //заглушка, т/к/ в API нет метода для получения отзывов)))
        ...
        let reviewUser = ReviewUser(text: "Test1", user: User(id: 1, login: "login", name: "name", lastName: "last name"))
        reviewList.append(reviewUser)
